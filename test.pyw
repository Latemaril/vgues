...
def clicked(): # Создаём функцию для вывода текста после клика по кнопке
    lbl.configure(text='Короче тут я пока ничего не придумал', fg='blue') # Меняем настройки текста с помощью функции configure(настроить)
...
def clicked1(): # Создаём функцию для отображения введённого текста с некоторым словом(В данном случае перед текстом будет Гачи)
    res = "Гачи {}".format(txt.get()) # Можно получить текст ввода, использую функцию get(непонятная херня так что лучше просто запомнить)
...
lbl = Label(root, text='Здарова славяне', fg='purple', font=('Arial Bold', 60)) # Создаём текст в окне с помощью класса Label.fg - цвет текста, font - шрифт
